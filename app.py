...
def render_sidebar():
    with st.sidebar:
        ...
        # Botão verificar imagens
        can_check = (
            st.session_state.get("aoi_geojson") is not None
            and st.session_state.get("selected_analysis") != "__custom__"
            and st.session_state.get("selected_satellite") is not None
        )
        if st.button("🔍 Verificar imagens disponíveis",
                     disabled=not can_check, use_container_width=True):
            _check_image_count()
